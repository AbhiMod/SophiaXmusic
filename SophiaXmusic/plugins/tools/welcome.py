...
def welcomepic(pic, user, chatname, id, uname):
    background = Image.open("SophiaXmusic/assets/bg2.jpg")
    pfp = Image.open(pic).convert("RGBA")
    pfp = circle(pfp)
    pfp = pfp.resize((450, 450))
    draw = ImageDraw.Draw(background)
    font = ImageFont.truetype('SophiaXmusic/assets/font.ttf', size=40)
    welcome_font = ImageFont.truetype('SophiaXmusic/assets/font.ttf', size=60)
...
async def greet_group(_, member: ChatMemberUpdated):
    ...
    try:
        ...
        pic = "SophiaXmusic/assets/bg2.jpg"
...
➖➖➖➖➖➖➖➖➖➖➖➖**
""",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f"⦿ ᴀᴅᴅ ᴍᴇ ⦿", url=f"https://t.me/Sophia_x_MusicBot?startgroup=true")]])
        )
    except Exception as e:
        LOGGER.error(e)
    try:
